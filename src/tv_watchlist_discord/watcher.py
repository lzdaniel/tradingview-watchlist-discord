...
def chunk_lines(lines: Sequence[str], limit: int = MAX_DISCORD_CONTENT) -> List[str]:
    chunks: List[str] = []
    current = ""
    for line in lines:
...
def send_discord(config: Config, content: str) -> None:
    ...
    request = urllib.request.Request(
        ...
        data=payload,
        headers={"Content-Type": "application/json", "User-Agent": "tv-watchlist-discord/0.1"},
        method="POST",
    )
    try:
...
def build_diff_message(config: Config, added: Sequence[WatchItem], removed: Sequence[WatchItem], now: datetime) -> List[str]:
    lines = [
        f"🚨 **{config.watchlist_name} Watchlist 更新** 🚨",
        f"🕒 {now.strftime('%Y-%m-%d %H:%M:%S %Z')}",
        f"📊 🟢 新增: {len(added)} | 🔴 移除: {len(removed)}",
        "",
    ]
    if added:
        lines.append(f"🟢🆕 **新加入 {len(added)} 个**")
        lines.extend(format_grouped_items(added, "🟢 +"))
        lines.append("")
    if removed:
        lines.append(f"🔴🗑️ **已移除 {len(removed)} 个**")
...
def build_full_message(
    ...
    zh_label = "开盘完整列表" if label == "open" else "收盘完整列表" if label == "close" else "完整列表"
    lines = [
        f"{emoji} **{config.watchlist_name} - {zh_label}**",
        f"🕒 {now.strftime('%Y-%m-%d %H:%M:%S %Z')}",
        f"📊 🟢 新增: {added_count} | 🔴 移除: {removed_count}",
        f"📌 共 {len(items)} 个 symbols",
        "",
    ]
...
def run_once(config: Config, force_full_label: Optional[str] = None) -> Dict[str, Any]:
    ...
    if force_full_label:
        ...
        print(f"No Discord notification needed. symbols={len(items)} added={len(added)} removed={len(removed)}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Monitor a TradingView public watchlist and notify Discord on changes.")
    subparsers = parser.add_subparsers(dest="command", required=True)

    once = subparsers.add_parser("run-once", help="Fetch once, diff against state, and notify only if needed.")
    once.add_argument("--dry-run", action="store_true", help="Print Discord messages instead of sending them.")

    daemon = subparsers.add_parser("daemon", help="Run forever with market/off-hours intervals.")
    daemon.add_argument("--dry-run", action="store_true", help="Print Discord messages instead of sending them.")
    ...
    scheduled.add_argument("--dry-run", action="store_true", help="Print Discord messages instead of sending them.")
    ...
    full.add_argument("--dry-run", action="store_true", help="Print Discord messages instead of sending them.")
    return parser
